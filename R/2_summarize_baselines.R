...
si %>% filter(!is.na(sitecode)) %>%
  ...
  ggplot(aes(x = dh2, y = d15n, color  = role, group = sitecode))+
  geom_point(size = 3)+
  theme_bw()+
  facet_wrap(~sitecode)
...
#check sc baselines
cc = si %>% select(role, sitecode, delta2h, delta15n)
...
si_sc <- rbind(cc, ter_base_sc, aq_base_sc)

ggplot(si_sc, aes(x = delta2h, y = delta15n, color= role, group = sitecode))+
  geom_point(size = 3)+
  theme_bw()+
  facet_wrap(~sitecode)
...
si_pp <- rbind(cc, ter_base_pp, aq_base_pp)

ggplot(si_pp, aes(x = delta2h, y = delta15n, color= role, group = sitecode))+
  geom_point(size = 3)+
  theme_bw()+
  facet_wrap(~sitecode)
