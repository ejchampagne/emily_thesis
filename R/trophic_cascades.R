...
# Normalize stomach row
sitefish <- transform(sitefish, stomach= tolower(stomach))
sitefish <- transform(sitefish, algae= ifelse(grepl("algae", stomach, TRUE), 1, 0))
# Total counts grouped by sitecode
sitefish %>% group_by(sitecode) %>% tally()

# Total Algae grouped by sitecode
sitefish %>% group_by(sitecode) %>% filter(algae==1) %>% count()

# Total cc kept grouped by sitecode
sitefish %>% group_by(sitecode) %>% filter(kept=="Y") %>% count()
